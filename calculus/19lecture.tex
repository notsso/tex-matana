...
\begin{proof}
	...
	\[\l|\bsc_{k=1}^NP_k^{-\eps}\r|=\sum_{k=1}^N|P_k^{-\eps}|=(1-\eps)\sum_{k=1}^N=(1-\eps)|P_k|=(1-\eps)|M|\text{ (для }|M|\neq0)\]
	...
	\[M=\bsc_{i=1}^\i M_i\Ra|M|=\sum_{i=1}^\i|M_i|,\ M\subset\sum_{i=1}^\i M_i\Ra|M|\le\sum_{i=1}^\i|M_i|\]
	\[\sum_{i=1}^\i|M_i|\le|M|,\ \lim_{m\ra\i}\bsc_{i=1}^mM_i\subset M\Ra\sum_{i=1}^m|M_i|\le|M|\Ra\]
	Получили требуемое.
\end{proof}