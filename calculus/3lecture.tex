...
\begin{theorem}[Больцано-Коши в $\R^n$]
...
\section{Дифференцируемость функций нескольких переменных}
Пусть $f$ определена на $U(\v x_0),\ \v x_0\in \R^n$ и $f(U(\v x_0))\subset\R$.
\begin{definition}
	$f$ называется \textit{дифференцируемой} в точке $\v x_0$, если $(\exists\v A\in\R^n)$ такой, что (полное) приращение $f$ в точке $\v x_0,\ \tr y=f(\tr\v x+\v x_0)-f(\v x_0)$ представимо в виде $\tr y=(\v A, \tr \v x)+\sigma{|\tr\v x|},\ \tr\v x\ra\v 0$, где $(\v a,\v b)$ - скалярное произведение.
\end{definition}
\begin{definition}
...
\begin{note}[Лирическое отступление Лукашова]
	Как - то раз я в Саратове лекцию читал, мне коллега рассказал, что такой же потолок как в Б.Физ. обваливался.
\end{note}
	
\begin{definition}
	Частичным (частным) приращением функции $f$ в точке $\v x_0=(x_{1,0},x_{2,0},\dots,x_{n,0})$ по переменной $x_j$ называется \[\tr_jf=f(\v x_0+\tr_j\v x)-f(\v x_0)\text{, где }\tr_j\v x=(0,\dots,\ 0,\ \tr x_j,\ 0,\dots,0),\ j=1,\dots,n\]
...
\begin{note}
	Обратное к теореме{~\eqref{th1}} неверно.
\end{note}
\begin{example}
	\[f(x,y)=
	\begin{cases}
...
\end{proof}
\begin{note}
	Обратное к{~\eqref{th2}} неверно, см. пример{~\eqref{ex1}}.
\end{note}
\begin{note}
	Теоремы{~\eqref{th1}} и{~\eqref{th2}} дают необходимое условие дифференцируемости.
\end{note}
\begin{theorem}[Достаточное условие дифференцируемости]\label{th3}
...
\begin{proof}
	...
	\end{multline*}
	Применим теорему Лагранжа много раз: $\xi_i\in\ [x_{i,0},x_{i,0}+\tr x_i],\ i=1,\dots,n$ (если $\tr x$ отрицателен, думаю поймёте).
	\begin{multline*}
...
\end{proof}
\begin{note}
	Условие теоремы{~\eqref{th3}} не является необходимым.
\end{note}
\begin{example}
	$f(x,y)=\sqrt[3]{x^2y}$
\end{example}