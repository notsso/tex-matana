...
\begin{theorem}\nf{(Мертенса)}\label{14_th16}
	Если хотя бы 1 из 2 рядов $\ss a_n$ и $\ss b_n$ сходится абсолютно, то их произведение по Коши $\ss c_n=\sum_{k=1}^{n}a_kb_{n-k+1}$ сходится, причём к произведению сумм этих рядов.
\end{theorem}
\begin{proof}
...
\end{definition}
\begin{definition}
	Функциональная последовательность $\{f_n(x)\}_{n=1}^\i$ называется сходящейся поточечно, если:
...
\begin{proof}
	...
	\[(\fa x\in E)\ f_n(x)\ra f(x)\text{ при }n\ra\i\]
