\begin{theorem}\nf{(Предельный переход в равномерно сходящихся рядах)}\label{16_th5}\\
...
\begin{proof}
	...
	\begin{multline*}
		...
		\B|\big(f_{n+p}(x)-f_n(x)\big)-\big(f_{n+p}(x_0)-f_n(x_0)\big)\B|+\B|f_{n+p}(x_0)-f_{n+p}(x_0)\B|\le\\
		\le|f'_{n+p}(\xi_1)-f'_n(\xi_1)|\cdot(b-a)+\B|f_{n+p}(x_0)-f_n(x_0)\B|<\eps
	\end{multline*}
	Получили:
...
\begin{proof}
	...
	\end{multline*}
	\[(\fa x\in\R)(\fa n\in\N)\ \al_m\le x\le \beta_m,\ \beta_m-\al_m\ra0,\ \frac{f(\beta_m)-f(\al_m)}{\beta_m-\al_m}\ra\i\text{ при }m\ra\i\]
	От противного: пусть $\ex$ производная $f'(x)$ в точке $x\in\R$:
	\begin{equation*}
		\begin{split}
			\frac{f(\beta_m)-f(\al_m)}{\beta_m-\al_m}&=\frac{f(\beta_m)-f(x)}{\beta_m-\al_m}+\frac{f(x)-f(\al_m)}{\beta_m-\al_m}=\\
			&=\frac{\beta_m-x}{\beta_m-\al_m}\cdot\l(\frac{f(\beta_m)-f(x)}{\beta_m-x}-f'(x)\r)+\\
			&+\frac{x-\al_m}{\beta_m-\al_m}\cdot\l(\frac{f(x)-f(\al_m)}{\beta_m-\al_m}-f'(x)\r)+f'(x)\ra f'(x)\text{ при }m\ra\i
		\end{split}
	\end{equation*}
	Отметим, что выражение чуть выше имеет место, только когда $\al_m<x<\beta_m$.\\
	Если одно из неравенств нестрогое, то б. о. о. $x=\al_m,\ \beta_m-x=1$, одно из слагаемых будет равно 0.
	Получили требуемое.
\end{proof}