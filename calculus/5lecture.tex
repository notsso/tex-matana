...
\end{examples}
\begin{definition}
	Функция называется \textit{$m$ раз дифференцируемой} в точке $\v x_0$, если все её частные производные порядка $m-1$ определены в окрестности точки $\v x_0$ и дифференцируемы в ней.
\end{definition}
\begin{definition}
