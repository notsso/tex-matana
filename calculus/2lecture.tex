...
\begin{theorem}
	Если существует конечный предел $b$ функции $f$ по совокупности переменных, то:
	\[(\fa\v d\ne0)(\vx \in D_{\v d})\ \lim_{\vx\ra\vx_0}f(x)=b\]
\end{theorem}
\begin{proof}
...
\begin{definition}
	Если $\v x_0$ - предельная точка области определения $D(f)$, то непрерывность $f$ в точке $\vx_0$ равносильна $\lim_{\vx\ra\vx_0}f(\vx)=f(\v x_0)$.\\
	Если $\v x_0$ - изолированная точка $D(f)$, то $f$ непрерывна в $\v x_0$.\\
...
\end{proposition}
\begin{proof}
	\[(\forall x\in G)\exists U^A_\eps(x)\subset G\Ra\mathfrak{G}:=\bigcup_{x\in G}U_\eps(x)\]
	Получили требуемое.
\end{proof}
