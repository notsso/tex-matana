...
\begin{note}
	...
	\]
\end{note}
\begin{theorem}\nf{(Свойства интеграла с переменным верхним пределом)}\label{9_th2}\\
...
\begin{note}[Лирическое отступление]
	Откуда появилось обозначение интеграла?\\
...
\end{note}
\begin{theorem}\nf{(Основная теорема интегрального исчисления)}\label{9_th3}\\
	Если $f\in\mc R[a,b]$ и имеет первообразную $F$ на этом отрезке, то справедлива формула Ньютона - Лейбница:
...
\begin{proof}
	...
	\begin{multline*}
		F(b)-F(a)=F'(t_{n})(b-x_{n-1})+\dots+F'(t_1)(x_1-a)=\\
		=\sum_{i=1}^nf(t_i)\tr x_i=S(P,f,\{t_i\})\ra\int_a^bf(x)dx\text{, при }\tr(P)\ra0
	\end{multline*}
