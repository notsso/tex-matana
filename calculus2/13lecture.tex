\begin{definition}
	\textit{Метод суммирования Чезаро}. 
	\[\sum_{n=1}^\infty a_n,\ S_n=\sum_{k=1}^na_n,\ G_n:=\frac{S_1+\dots+S_n}{n}\]
	Если $\ex\lim_{n\ra\infty}G_n=S$, то $S$ называется суммой по Чезаро $\sum_{n=1}^\infty a_n$.
\end{definition}
...
\end{proof}
\begin{corollary}
	Если $\ds\lim_{n\ra\i}\frac{a_{n+1}}{a_n}=l$, то $\lim_{n\ra\i}\sqrt[n]{a_n}=l$.
\end{corollary}
\begin{proof}
...
\begin{proof}
	По критерию Коши:
	\begin{multline*}
		\sum_{k=n+1}^{n+p}a_kb_k=(A_k-A_{k-1})b_{n+1}+\dots+(A_{n+p}-A_{n+p-1})b_{n+p}=\\
		=A_{n+p}b_{n+p}-A_nb_{n+1}+\sum_{k=n+1}^{n+p-1}A_k(b_k-b_{k+1})
	\end{multline*}
	\[|A_N|\le M\ (\fa N\in\N)\]
	\[\l|\sum_{k=n+1}^{n+p}a_kb_k\r|\le M\l(|b_{n+p}|+|b_{n+1}|+\l|\sum_{k=n+1}^{n+p-1}(b_k-b_{k+1})\r|\r)\le2M(|b_{n+1}|+|b_{n+p}|)\]
...
\begin{corollary}(\nf{Признак Абеля сходимости числовых рядов})\\
	...
	\begin{enumerate}
		...
		\item $\{b_n\}_{n=1}^\i$ монотонна и ограничена.
	\end{enumerate}
	то $\ds\ss a_nb_n$ cходится.
\end{corollary}
\begin{proof}
...
\end{proof}
\begin{examples}
	\[\ss\frac{\sin nx}{n^\al},\ \ss\l(1+\frac1n\r)^n\cdot\frac{\sin nx}{n^\al},\ x\ne\pi n,\ n\in\Z\]
	...
	$\ds\l\{\frac1{n^\al}\r\}$ - монотонная и бесконечно мала при $\al>0$.\\
	$\ds\ss\frac{\sin nx}{n^\al}$ сходится при $\al>0$.
	...
	\end{equation*}
\end{examples}
\begin{theorem}\nf{(О перестановках абсолютно сходящихся рядов)}\label{13_th13}\\
	Если ряд $\ss a_n$ сходится абсолютно, то $\fa$ биекций $n:\N\ra\N$ и последовательности $\{n_k\}_{k=1}^\i,\ n_k=n(k)$ ряд $\sum_{k=1}^\i a_{n_k}$ сходится к той же сумме, что и $\ss a_n$.
\end{theorem}
\begin{proof}
	Положим $a_k^*:=a_{n_k}$\\
