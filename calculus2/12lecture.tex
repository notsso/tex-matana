...
\begin{lemma}\nf{(Элементарные свойства рядов)}\\
	\begin{enumerate}
		...
		\item $(\fa n_0\in\N)$ ряды $\ds\sum_{n=1}^\i a_n$ и $\ds\sum_{n=n_0}^\i a_n$ сходятся или расходятся одновременно.
	\end{enumerate}
\end{lemma}
\begin{proof}
...
\end{theorem}
\begin{proof}
	\[\ds\fa x\in[k,k+1]\ f(k)\ge f(x)\ge f(k+1),\ f(k)\ge\int_k^{k+1}f(x)dx\ge f(k+1)\]
	\[\sum_{k=1}^nf(k)\ge\int_k^{k   +1}f(x)dx\ge\sum_{k=1}^nf(k+1)=\sum_{j=2}^{n+1}f(j)\]
	Получили требуемое.
\end{proof}
\begin{example}
...
\begin{theorem}\label{12_th5}\nf{(Признак Даламбера)}\\
	...
	\begin{enumerate}
		\item $\ds\vlim[n\ra\i]\frac{a_{n+1}}{a_n}=p<1$, тогда ряд $\ds\ss a_n$ сходится.
		\item $\ds\uvlim[n\ra\i]\frac{a_{n+1}}{a_n}=q>1$, тогда ряд $\ds\ss a_n$ расходится.
	\end{enumerate}
\end{theorem}
\begin{proof}
...
\begin{theorem}\label{12_th6}\nf{(Признак Коши)}\\
	Пусть $a_k>0\ \fa k\in\N$. Тогда:
	\begin{enumerate}
		\item $\ds\vlim[n\ra\p]\sqrt[n]{a_n}<1$, тогда $\ds\ss a_n$ сходится.
		\item $\ds\vlim[n\ra\p]\sqrt[n]{a_n}>1$, тогда $\ds\ss a_n$ расходится.
	\end{enumerate}
\end{theorem}
\begin{proof}
	Докажем первый пункт.
	\[p:=\vlim[n\ra\p]\sqrt[n]{a_n},\ (\fa\eps>0)(\ex N\in\N)(\fa n>N)\ \sqrt[n]{a_n}<p+\eps=\t p<1,\ a_n<(\t p)^n,\ \ss[N+1](\t p)^n\text{ сходится}\]
