...
\begin{proof}
	...
	\begin{equation*}
		\begin{split}
			U(P_1,f)&=\dots+\smo{\sup_{\sus[x_{k-1},x^*]}}f(x)\cdot(x^*-x_{k-1})+\smo{\sup_{\sus[x^*,x_k]}}f(x)\cdot(x_k-x^*)+\dots\\
	...
	Тогда имеем: 
	\[U(P_2,f)-U(P_1,f)=(x^*-x_{k-1})\cdot(\smo{\sup_{\sus[x_{k-1},x_{k}]}}f(x)-\smo{\sup_{\sus[x_{k-1},x^*]}}f(x))+(x_k-x^*)\cdot(\smo{\sup_{\sus[x_{k-1},x_{k}]}}f(x)-\smo{\sup_{\sus[x^*,x_k]}}f(x))\ge0\]
	Вторая часть леммы доказывается аналогично. Получили требуемое.
\end{proof}
\begin{theorem}\nf{(Основное свойство сумм Дарбу)}\\
...
\end{definition}
\begin{theorem}\nf{(Критерий интегрируемости)}\label{7_th2}\\
...
\begin{proof}
	...
	\[U(P_1,f)<I(f)+\frac\eps2,\ L(P_2,f)>I(f)-\frac\eps2\]
	...
	\[L(P,f)\le \uv I(f)\le\v I(f)<U(P,f)\Ra \v I(f)-\uv I(f)<\eps\]
	Получили требуемое.
\end{proof}
\begin{theorem}
...
\end{examples}
\begin{theorem}\nf{(Основные свойства интеграла Римана)}
	\begin{enumerate}
		\item Линейность. Если $f_1,\ f_2\in\mc R[a,b],\ c\in\R$, то:
		\begin{enumerate}
			\item $(f_1+f_2)\in\mc R[a,b],\ cf_1\in\mc R[a,b]$\\
			\item $\displaystyle\int_a^b(f_1+f_2)(x)dx=\int_a^bf_1(x)dx+\int_a^bf_2(x)dx$\\
			\item $\displaystyle\int_a^bcf(x)dx=c\int_a^bf(x)dx$
		\end{enumerate}
...
\end{theorem}
\begin{proof}
	Докажем первый и третий пункты.\\
	\begin{enumerate}
	\item $f_1,f_2\in\mc R[a,b]\Ra^?(f_1+f_2)\in\mc R[a,b]$\\
	По критерию интегрируемости (\ref{7_th2}):
	\[(\forall\eps>0)(\exists P_i)L(P_i,f_i)\le\int_a^bf_i(x)dx\le U(P_i,f_i),\ U(P_i,f_i)-L(P_i,f_i)<\frac\eps2\] 
	Возьмём $P'=P_1\cup P_2$.
	\begin{equation}\label{7_eq1}
		L(P,f_i)\le\int_a^bf_i(x)dx\le U(P',f_i),\ U(P',f_i)-L(P',f_i)<\frac\eps2\\
	\end{equation}
	\[U(P',f_1+f_2)=\sum_{i=1}^nM_i(f_1+f_2)\tr x_i\le\sum_{i=1}^n(M_i(f_1)+M_i(f_2))\tr x_i=U(P',f_1)+U(P',f_2)\]
	\[M_i(f_1+f_2)=\smo{\sup_{\hspace{0.1cm}[x_{i-1},x_i]}}(f_1+f_2)\le\smo{\sup_{\hspace{0.1cm}[x_{i-1},x_i]}}f_1+\smo{\sup_{\hspace{0.1cm}[x_{i-1},x_I]}}f_2=M_i(f_1)+M_i(f_2)\]
	\[L(P',f_1+f_2)\ge L(P',f_1)+L(P',f_2)\]
	По (\ref{7_eq1}) имеем:
	\[U(P',f_1+f_2)-L(P',f_1+f_2)<\eps\Ra (f_1+f_2)\in\mc R[a,b]\]
	Теперь докажем пункт $(1)$.\\
	\[L(P',f_1)+L(P',f_2)\le\int_a^bf_1(x)dx+\int_a^bf_2(x)dx\le U(P',f_1)+U(P',f_2)\]
	\begin{multline*}
		L(P',f_1+f_2)\le\int_a^b(f_1+f_2)(x)dx\le U(P',f_1+f_2)\Ra\\
		\Ra\bigg|\int_a^b(f_1+f_2)(x)dx-\bigg(\int_a^bf_1(x)dx+\int_a^bf_2(x)dx\bigg)\bigg|<\eps
	\end{multline*}
	\item[3.] $\exists c: a<c<b,$\\ 
	Хотим доказать: $\displaystyle\int_a^bf(x)dx=\int_a^bf(x)dx+\int_c^bf(x)dx$
	\[f\in\mc R[a,b]\lra(f\in\mc R[a,c]\land f\in\mc R[a,b])\Ra(\eps>0)(\exists P)\ U(P,f)-L(P,f)<\eps\]
	$c\in P\Ra P=P_1\cup P_2,\ P_1:a=x_0<\dots<x_k=c,\ P_2:c=x_0<\dots<x_n=b$\\
	\[U(P_i,f)-L(P_i,f)<\eps,\ i=1,2\Ra (f\in\mc R[a,c]\land f\in\mc R[c,b])\]
	\begin{multline*}
		L(P_1,f)+L(P_2,f)=L(P,f)\le\int_a^bf(x)dx\le U(P,f)=U(P_1,f)+U(P_2,f)\Ra\\
		\Ra\bigg|\int_a^bf(x)dx-(\int_a^bf(x)+\int_c^bf(x)dx)\bigg|<\eps
	\end{multline*}
	Все переходы $\lra$, значит можно их применить в обратном порядке.\\ 
	Доказали требуемое.
	\end{enumerate}
\end{proof}